...
/*void draw_map(t_cube *cubed)
{
    int start_y = (int)cubed->y - MINIMAP_Y;
    int end_y = (int)cubed->y + MINIMAP_Y;
    int start_x = (int)cubed->x - MINIMAP_X;
    int end_x = (int)cubed->x + MINIMAP_X;
    int map_height;
    ...
    for (int y = start_y; y <= end_y; y++, draw_y++)
    {
        ...
        for (int x = start_x; x <= end_x; x++, draw_x++)
        {
            ...
            if (y >= 0 && cubed->map[y] && x >= 0 && x < ft_strlen(cubed->map[y]))
            {
                ...
                int screen_x = draw_x * SIZE * MINIMAP_SIZE;
                int screen_y = draw_y * SIZE * MINIMAP_SIZE;
